...
export function TripCompletionCard({
  ...
  const handleSubmit = () => {
    Alert.alert(
      'Đã ghi nhận đánh giá demo',
      'GoRide sẽ nối POST /ratings ở bước backend tiếp theo. Cảm ơn bạn đã phản hồi!',
    );
  };
    ...
    <View style={styles.card}>
      ...
      <View style={styles.ratingBox}>
        ...
        <View style={styles.starRow}>
          {[1, 2, 3, 4, 5].map((score) => (
            <TouchableOpacity key={score} activeOpacity={0.78} onPress={() => setRating(score)}>
        ...
        <TouchableOpacity activeOpacity={0.84} style={styles.submitButton} onPress={handleSubmit}>
          <MaterialCommunityIcons name="send-check-outline" size={rs(24)} color={palette.card} />
          <Text style={styles.submitText}>Gửi đánh giá demo</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
